// Automations (Scheduled) tools: list, read one with its run history, create,
// update, pause/resume, run now and delete. Every tool calls the SAME endpoints
// the Automations tab calls (/api/agent/automations…, the runtime's automations
// store) — there is deliberately no state here: if a tool and the tab ever
// showed different automations, a second store would have been introduced.
//
// Ported from pi-extensions/automations.ts; names, descriptions and schemas unchanged.

import { Type } from "@earendil-works/pi-ai";
import { failure, type HarnessTool, textResult, type ToolContext, type ToolResult, withTimeout } from "./context";

const CALL_TIMEOUT_MS = 30_000;
// "Run now" is not a store write: the endpoint runs the whole automation turn in
// a fresh session and only answers once the result has been recorded, so it gets
// the subagent-sized budget instead of the 30s call budget.
...
type ScheduleArg = { kind?: unknown; minutes?: unknown; time?: unknown; day?: unknown; weekdaysOnly?: unknown };
...
/** Validate the agent-supplied schedule into the store's shape, or explain what is
 *  wrong. The store silently rewrites anything it cannot parse to "daily at 08:00",
 *  so every write path must pass through here first. */
export function normalizeScheduleArg(
  ...
  if (kind === "interval") {
    ...
    if (!Number.isFinite(minutes) || minutes < 1) return { ok: false, error: "interval schedule needs 'minutes' >= 1." };
    return { ok: true, schedule: { kind: "interval", minutes } };
  }
  if (kind === "daily") {
    if (!isValidTime(input.time)) return { ok: false, error: "daily schedule needs 'time' as 'HH:MM' (24h)." };
    return {
      ok: true,
      schedule: { kind: "daily", time: (input.time as string).trim(), ...(input.weekdaysOnly === true ? { weekdaysOnly: true } : {}) },
    };
  }
  if (kind === "weekly") {
    const day = typeof input.day === "number" ? Math.round(input.day) : NaN;
    if (!Number.isInteger(day) || day < 0 || day > 6) return { ok: false, error: "weekly schedule needs 'day' 0-6 (0 = Sunday)." };
    if (!isValidTime(input.time)) return { ok: false, error: "weekly schedule needs 'time' as 'HH:MM' (24h)." };
...
export function describeSchedule(schedule: NormalizedSchedule): string {
  ...
  if (schedule.kind === "daily") return `daily at ${schedule.time}${schedule.weekdaysOnly ? " (weekdays)" : ""}`;
...
async function httpJson(
  ...
  try {
    const response = await ctx.request(path, { ...init, signal: bounded.signal });
...
async function guarded(label: string, run: () => Promise<ToolResult>): Promise<ToolResult> {
  try {
    ...
    return failure(`Failed to ${label}: ${error instanceof Error ? error.message : String(error)}`);
  }
}

/** Explicit arg, else the current session's model, else the first available. */
async function resolveModelId(ctx: ToolContext, explicit: string | undefined, signal: AbortSignal | undefined): Promise<string | null> {
  const trimmed = explicit?.trim();
  if (trimmed) return trimmed;
  if (ctx.modelId.trim()) return ctx.modelId.trim();
  const { ok, body } = await httpJson(ctx, "/api/agent/models", { method: "GET" }, signal);
...
/** Explicit arg, else the current session's cwd, else the app default. */
function resolveCwd(ctx: ToolContext, explicit: string | undefined): string {
  return explicit?.trim() || ctx.env.LOCAL_STUDIO_CWD?.trim() || "";
}

// ─── Record formatting ──────────────────────────────────────────────────────

type RunRecord = { at?: unknown; outcome?: unknown; summary?: unknown; error?: unknown; piSessionId?: unknown };
...
function describeRunOutcome(run: RunRecord | null): string {
  ...
  return `last run ${run.outcome === "error" ? "failed" : "ok"} at ${text(run.at, "unknown time")}`;
}
...
function formatRunEntry(run: RunRecord, index: number): string {
  const lines = [`  ${index + 1}. ${text(run.at, "unknown time")} — ${run.outcome === "error" ? "error" : "ok"}`];
...
function formatLastRunBlock(run: RunRecord | null): string[] {
  ...
  const lines = [`last run: ${run.outcome === "error" ? "error" : "ok"} at ${text(run.at, "unknown time")}${session ? ` (session ${session})` : ""}`];
  const error = text(run.error);
...
function formatAutomationDetail(record: AutomationRecord, includeRuns: boolean): string {
  ...
  if (includeRuns && runs.length > 0) {
    lines.push(`run history (${runs.length} kept, newest first):`, runs.map(formatRunEntry).join("\n"));
  }
  return lines.join("\n");
}
...
/** The list endpoint is the only read path (there is no GET /automations/:id) and
 *  it already returns each automation in full, run history included. */
async function fetchAutomations(ctx: ToolContext, signal: AbortSignal | undefined): Promise<ListResult> {
  const { ok, status, body } = await httpJson(ctx, "/api/agent/automations", { method: "GET" }, signal);
...
async function fetchAutomation(
  ...
  const listed = await fetchAutomations(ctx, signal);
  if (!listed.ok) return listed;
  const match = listed.automations.find((record) => record.id === id);
  if (!match) return { ok: false, error: `No automation with id '${id}'. Call list_automations for the current ids.` };
...
const idParam = Type.String({ description: "The automation id from list_automations." });
const idExampleParam = Type.String({ description: "The automation id from list_automations, e.g. 'auto-1a2b3c4d'." });

// ─── Tools ──────────────────────────────────────────────────────────────────

export function automationsTools(ctx: ToolContext): HarnessTool[] {
  return [
    {
      ...
      execute: (_id, _params, signal) =>
        guarded("list automations", async () => {
          const listed = await fetchAutomations(ctx, signal);
          if (!listed.ok) return failure(`Failed to list automations: ${listed.error}`);
          if (listed.automations.length === 0) {
            return textResult("No automations are scheduled. Use schedule_automation to create one.", { count: 0 });
          }
    ...
    },
    {
      ...
      parameters: Type.Object({
        id: idExampleParam,
        includeRuns: Type.Optional(Type.Boolean({ description: "Include the run history. Default true." })),
      }),
      async execute(_id, params, signal) {
        ...
        return guarded("read automation", async () => {
          const found = await fetchAutomation(ctx, parsed.id, signal);
    ...
    },
    {
      ...
      parameters: Type.Object({
        ...
        model: Type.Optional(Type.String({ description: "Model id; defaults to the current session's model." })),
        cwd: Type.Optional(Type.String({ description: "Working directory; defaults to the current project." })),
        sessionId: Type.Optional(Type.String({ description: "Existing pi session to run inside; omit for a fresh session each run." })),
      }),
      async execute(_id, params, signal) {
        const args = (params ?? {}) as { prompt?: string; schedule?: ScheduleArg; name?: string; model?: string; cwd?: string; sessionId?: string };
        ...
        return guarded("create automation", async () => {
          const modelId = await resolveModelId(ctx, args.model, signal);
          if (!modelId) return failure("No model available to run the automation. Pass a 'model' id.");
    ...
    },
    {
      ...
      parameters: Type.Object({
        ...
        sessionId: Type.Optional(
          Type.String({ description: "Pi session every run should continue; empty string goes back to a fresh session." }),
        ),
      }),
      async execute(_id, params, signal) {
        const parsed = requireId(params, "update_automation");
        if (!parsed.ok) return failure(parsed.error);
        const args = (params ?? {}) as { name?: unknown; prompt?: unknown; schedule?: ScheduleArg; model?: unknown; cwd?: unknown; sessionId?: unknown };
        ...
        // An empty string is a real instruction here — "stop running in that session".
        ...
        if (Object.keys(patch).length === 0) {
          return failure("update_automation needs at least one field to change (name, prompt, schedule, model, cwd or sessionId).");
        }
        return guarded("update automation", async () => {
          const { ok, status, body } = await httpJson(ctx, automationPath(parsed.id), jsonInit("PATCH", patch), signal);
    ...
    },
    {
      ...
      async execute(_id, params, signal) {
        ...
        if (status !== "active" && status !== "paused") return failure("set_automation_status needs status 'active' or 'paused'.");
        return guarded("change automation status", async () => {
          const reply = await httpJson(ctx, automationPath(parsed.id), jsonInit("PATCH", { status }), signal);
          if (!reply.ok) return failure(`Failed to change automation status: ${errorText(reply.body, reply.status)}`);
    ...
    },
    {
      ...
      async execute(_id, params, signal) {
        ...
        return guarded("run automation", async () => {
          const { ok, status, body } = await httpJson(ctx, automationPath(parsed.id, "/run"), { method: "POST" }, signal, RUN_TIMEOUT_MS);
          if (!ok) return failure(`Failed to run automation: ${errorText(body, status)}`);
          if ((body as { started?: unknown })?.started !== true) {
            return textResult(
    ...
    },
    {
      ...
      description:
        ...
        "'paused' instead.",
      parameters: Type.Object({ id: idExampleParam }),
      async execute(_id, params, signal) {
        const parsed = requireId(params, "delete_automation");
        if (!parsed.ok) return failure(parsed.error);
        return guarded("delete automation", async () => {
          const { ok, status, body } = await httpJson(ctx, automationPath(parsed.id), { method: "DELETE" }, signal);
