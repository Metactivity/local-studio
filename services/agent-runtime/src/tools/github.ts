// github — GitHub through the `gh` CLI already installed and signed in on this
// machine. Shelling out (argv array, no shell) keeps credentials in gh's own
// keyring and repo resolution in gh; commands that hand out credentials or
// destroy things are refused outright. Registered only where a gh binary exists
// (the runtime's gate); being signed in is what `github_status` asks at call time.
//
// Ported from pi-extensions/github.ts; names, descriptions and schemas unchanged.

import { execFile } from "node:child_process";
import { type Static, type TSchema, Type } from "@earendil-works/pi-ai";
import type { HarnessTool, ToolContext, ToolResult } from "./context";
...
/** cwd: what `gh` resolves OWNER/REPO from; bin: the binary the runtime found. */
type GithubEnv = { cwd: string; bin: string };

function readEnv(ctx: ToolContext): GithubEnv {
  return { cwd: ctx.env.LOCAL_STUDIO_CWD || ctx.cwd || process.cwd(), bin: ctx.env.LOCAL_STUDIO_GH_PATH || "gh" };
}

function runGh(env: GithubEnv, args: string[], signal: AbortSignal | undefined, timeoutMs = DEFAULT_TIMEOUT_MS): Promise<GhRun> {
  return new Promise((resolve) => {
    execFile(
      ...
      args,
      {
        ...
        // gh pipes into a pager and colourizes when it thinks it has a TTY; both corrupt what a model reads.
        env: { ...process.env, GH_PAGER: "cat", PAGER: "cat", NO_COLOR: "1", CLICOLOR: "0", GH_PROMPT_DISABLED: "1" },
      },
      (error, stdout, stderr) => {
        const code = error && typeof (error as { code?: unknown }).code === "number" ? ((error as { code: number }).code ?? 1) : error ? 1 : 0;
        resolve({
          code,
...
function refuse(reason: string): ToolResult {
  return { content: [{ type: "text", text: reason }], details: { refused: true, failed: true } };
}

// ─── shared parameters ────────────────────────────────────────────────────

const repoParam = Type.Optional(
  Type.String({ description: "OWNER/REPO. Omit to use the repository checked out in this session's project directory." }),
);
const limitParam = Type.Optional(Type.Number({ description: "Maximum results (default 30)" }));

const repoArgs = (repo: string | undefined): string[] => (repo ? ["--repo", repo] : []);
...
const TOOLS = [
  define({
    ...
    parameters: Type.Object({
      kind: Type.Union(
        [Type.Literal("issues"), Type.Literal("prs"), Type.Literal("repos"), Type.Literal("code"), Type.Literal("commits")],
  ...
  define({
    ...
    parameters: Type.Object({
      repo: repoParam,
      state: Type.Optional(
        Type.Union([Type.Literal("open"), Type.Literal("closed"), Type.Literal("all")], { description: "Issue state (default open)" }),
      ),
  ...
  define({
    ...
    parameters: Type.Object({
      ...
      repo: repoParam,
      comments: Type.Optional(Type.Boolean({ description: "Include the comment thread (default true)" })),
    }),
    argv: (p) => [
      "issue",
      "view",
  ...
  define({
    ...
    parameters: Type.Object({
      ...
      repo: repoParam,
      reviews: Type.Optional(Type.Boolean({ description: "Include reviews and comments (default true)" })),
    }),
    argv: (p) => [
      "pr",
      "view",
  ...
  define({
    ...
    parameters: Type.Object({
      number: Type.Number({ description: "Pull request number" }),
      repo: repoParam,
      namesOnly: Type.Optional(Type.Boolean({ description: "List changed file paths instead of the diff" })),
    }),
    argv: (p) => ["pr", "diff", String(Math.trunc(p.number)), ...repoArgs(p.repo), ...(p.namesOnly ? ["--name-only"] : [])],
  }),
  define({
  ...
  define({
    ...
    parameters: Type.Object({
      ...
      status: Type.Optional(Type.String({ description: "Filter by status/conclusion, e.g. failure, in_progress" })),
      limit: limitParam,
    }),
    argv: (p) => [
      "run",
  ...
  define({
    ...
    parameters: Type.Object({
      ...
      repo: repoParam,
      failedLogs: Type.Optional(Type.Boolean({ description: "Return the logs of failed steps instead of the job summary" })),
    }),
    argv: (p) => [
      "run",
      "view",
      ...
      ...repoArgs(p.repo),
      ...(p.failedLogs ? ["--log-failed"] : ["--json", "databaseId,displayTitle,workflowName,status,conclusion,headBranch,headSha,event,jobs,url"]),
    ],
  ...
  define({
    ...
    parameters: Type.Object({
      endpoint: Type.String({ description: "REST path, e.g. repos/{owner}/{repo}/releases?per_page=5" }),
      jq: Type.Optional(Type.String({ description: "jq expression applied to the response" })),
      paginate: Type.Optional(Type.Boolean({ description: "Follow pagination and concatenate pages" })),
    }),
    argv: (p) => ["api", p.endpoint, "--method", "GET", ...(p.jq ? ["--jq", p.jq] : []), ...(p.paginate ? ["--paginate"] : [])],
    timeoutMs: SLOW_TIMEOUT_MS,
  }),
];

// `gh` subcommands that hand out or replace credentials, or install and run
// third-party code. Nothing a task legitimately needs is behind these.
const BLOCKED_COMMANDS = new Set(["auth", "secret", "variable", "extension", "alias", "config", "gpg-key", "ssh-key"]);
...
export function blockedReason(args: string[]): string | null {
  ...
  if (command.startsWith("-")) return "github_cli takes a gh subcommand first, not a flag.";
...
export function githubTools(ctx: ToolContext): HarnessTool[] {
  ...
  return [
    {
      ...
      async execute(_id, _params, signal) {
        ...
        const repo = await runGh(env, ["repo", "view", "--json", "nameWithOwner,url,isPrivate,defaultBranchRef"], signal);
        const repoLine =
          repo.code === 0
            ? repo.stdout.trim()
        ...
        return {
          content: [
            {
              type: "text",
              text: truncate(`${auth.stdout.trim() || auth.stderr.trim() || "(no auth output)"}\n\nSession directory: ${env.cwd}\n${repoLine}`),
            },
          ],
          details: { command: "gh auth status", exitCode: auth.code, cwd: env.cwd, ...(auth.code === 0 ? {} : { failed: true }) },
        };
      },
    },
    ...TOOLS.map(
    ...
    ),
    {
      ...
      parameters: Type.Object({ args: Type.Array(Type.String(), { description: "Arguments after `gh`, one per element" }) }),
      async execute(_id, params, signal) {
        const args = ((params as { args?: unknown[] }).args ?? []).map(String);
